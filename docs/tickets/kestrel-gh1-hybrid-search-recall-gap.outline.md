...
At the data level, the gene symbol and the HGNC marker are already present on the conflated node, so the durable fix is not to re-attach them but to make them count in retrieval. Two paths exist. The first is de-conflation: apply selective conflation so the gene node remains distinct from the drug node, the strategy under discussion in `biolink/biolink-model#633`; this is the cleanest outcome but depends on the Babel and NodeNormalization clique-building layer that we ingest. The second, fully under our control, is a retrieval-layer correction: index the gene symbol and the HGNC marker with gene-appropriate salience so that an exact symbol match on an HGNC-bearing node is surfaced as a candidate for gene-category queries, rather than being outranked by species orthologs whose only advantage is that the symbol is their primary name. Either correction returns the node to the candidate set, after which biomapper2's existing HGNC re-ranking already selects the human node over the orthologs; the missing piece is purely retrieval, not selection. We recommend the retrieval-layer fix as the immediate measure because it is ours to make and addresses the behavior (exact gene-symbol matching) rather than a hand-maintained list of six.
